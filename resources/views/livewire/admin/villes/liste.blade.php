                        ...
                        <tbody>
                            @foreach ($villes as $ville )
                                <tr>
                                    ...
                                    <td class="text-center">{{$ville->cp}}</td>
                                    <td class="text-center">{{$ville->region->nom}}</td>
                                    <td class="text-center">{{$ville->desc}}</td>
                                    <td class="text-center">
                                        <button class="btn btn-link"
                                            wire:click="goToUpdate({{$ville->id}})">
                                            <i class="far fa-edit"></i> 
                                        </button>
                                        <button class="btn btn-link" 
                                            wire:click="confirmDelete('{{$ville->nom}}',{{$ville->id}})">
    ...
    window.addEventListener('showDeleteMessageCONFIRM',event=>{
        ...
        window.addEventListener('showsuccessMessageDELETE',event=>{
            Swal.fire({
                position: 'center',
                icon: 'success',
                title: event.detail.message || 'villes suprimé avec succès!',
