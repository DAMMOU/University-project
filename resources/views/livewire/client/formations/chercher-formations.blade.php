          ...
          <div class="container">
            <div class="row row-cols-3">
              @foreach($this->resultats as $resultat)
                <div class="col">
                    <a href="{{url($resultat->universite->initiale.'/'.
                              generateAcronym($resultat->etablissement->nom).'-'.urlencode(Str::slug($resultat->ville->nom)).'/formations'.
                              '/'.$resultat->id.'/'.generateAcronym($resultat->intitule))}}">

                      {{ $resultat->intitule." - ".$resultat->categorie->initiale 
                      ." - ".$resultat->ville->nom}}
                    </a>
                </div>
              @endforeach
